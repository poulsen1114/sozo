...
// Handle scroll on page load, fetch JSON data, and render content
document.addEventListener('DOMContentLoaded', function() {
    loadHeader();

    const targetId = sessionStorage.getItem('scrollToSection') || (window.location.pathname.includes('index.html') && window.location.hash ? window.location.hash.substring(1) : null);
    if (targetId && window.location.pathname.includes('index.html')) {
        setTimeout(() => {
            const targetElement = document.querySelector('#' + targetId);
            if (targetElement) {
                const headerHeight = document.querySelector('.fixed-header')?.offsetHeight || 0;
    ...
    if (reviewsContainer) {
        fetch('reviews.json')
            ...
            .then(reviews => {
                ...
                reviewsContainer.addEventListener('mouseenter', () => clearInterval(autoScrollTimer));
                reviewsContainer.addEventListener('mouseleave', () => autoScrollTimer = setInterval(autoScroll, scrollInterval));
                reviewsContainer.addEventListener('touchstart', () => clearInterval(autoScrollTimer));
                reviewsContainer.addEventListener('touchend', () => autoScrollTimer = setInterval(autoScroll, scrollInterval));
            })
            .catch(error => {
